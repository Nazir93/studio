...
export function userFacingTelegramHint(
  ...
  if (reason === "missing_env") {
    return "Не заданы токен или chat_id в коде (frontend/src/lib/telegram.ts).";
  }
  if (reason === "network") {
  ...
  if (d.includes("unauthorized")) {
    return "Токен бота недействителен — обновите TELEGRAM_BOT_TOKEN в telegram.ts или выпустите новый в @BotFather.";
  }
...
/**
 * Токен и chat_id задаются только здесь (process.env не используется).
...
export async function sendTelegramNotification(message: string): Promise<TelegramSendResult> {
  ...
  if (!botToken || !chatId) {
    console.warn("[TELEGRAM] Нет токена или chat_id в telegram.ts");
  ...
  try {
    ...
    if (!res.ok || data?.ok === false) {
      ...
      console.error(
        ...
        "| HTTP",
        res.status,
        "| Подсказка: /start боту в личке; для группы chat_id -100…; токен в BotFather при отзыве меняется"
      );
